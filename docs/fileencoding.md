...
## Overview
Encoding describes how text itself is encoded inside of a map file. Map files are text files with the .DAT extension and the file it broken up into sections with different meaning, but it is entirely textual data.  How each character is encoded in a text file is dependent on what that character is.
...
>MMDatUtil uses an enhanced series of tests that auto detect the encoding format for all text files if they do not have a BOM.
...
>During output, the default encoding is UTF8 with a BOM. If you want to remove support for all characters above 255, you may force the output to be windows code page by using -ansi switch. This may also give incorrect extended ANSI characters.
...
To output any UTF without a BOM, you need to use the -nobom option.
